...
export class RaffleDrawModel extends Model { }
RaffleDrawModel.init(
    {
        reward_value: {
...
UserModel.hasMany(RaffleDrawModel);
RaffleDrawModel.belongsTo(UserModel);
...
RaffleDrawModel.sync(option).then(() => {
    logger.info("Survey Reward table migrated");
    // Table created
});

withCursor(paginationOptions)(<any>RaffleDrawModel);
