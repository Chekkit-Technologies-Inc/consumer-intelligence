...
export class ProfileController extends BaseController {
    ...
    public getAppProfile = async (username: string) => {
        const user = await this._profileService.getAppProfile(username);
        return this.sendResponse(user);
    }
    /**
     * editProfile
    ...
    public editProfile = async (user: any, photo: any, profile: any) => {
        const updated = await this._profileService.editProfile(user, photo, profile);
